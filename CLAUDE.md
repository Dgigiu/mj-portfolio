...
- **Motion is allowed but quiet** (policy in the design brief, updated June 2026): `transform`/`opacity` only, under ~400ms or scroll-driven, never animate text, no carousels or scroll-jacking. Current set: hero parallax, hero scroll exit veil (a cobalt-to-navy curtain that climbs faster than scroll to wash the portrait as the hero leaves), faint card reveals (never from alpha 0), zoom dialog scale, card hover lift, nav underline ease-in. No entrance animations on the hero.
